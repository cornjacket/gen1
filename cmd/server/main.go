...
func main() {
	...
	go func() {
		...
		_ = server.Publish("direct/retained", []byte("retained DRT message"), true, 0)
		_ = server.Publish("direct/alternate/retained", []byte("some other DRT retained message"), true, 0)
	...
	/*
		go func() {
			for range time.Tick(time.Second * 3) {
				...
				//server.Log.Info("main.go waiting...")
			}
		}()
	*/
	go func() {
		...
		_ = server.Unsubscribe("direct/#", 2) //DRT - added to see if this turns off all subs. Yes, previously 2 subs were created, but only 1 was unsub'd. Now both are disabled.
	}()
